                ...
                if ('Notification' in window && Notification.permission === 'granted') {
                    try {
                        new Notification('🥚 Hora da Viragem!', {
                            body: 'Vire os ovos da incubadora agora.',
                            icon: 'png192.png',
...
function viewBatchDetails(id) {
    ...
    const hatchDate = getLocalDate(batch.startDate);
...
function saveBatch(event) {
    ...
    try {
        ...
        renderBatches();
        closeModal();
        showNotification('✅ Sucesso', batchId ? 'Lote atualizado!' : 'Lote salvo!');
    } catch (e) {
        showNotification('❌ Erro', 'Não foi possível salvar.');
    }
}
...
function deleteBatch(id) {
    if (confirm('Tem certeza que deseja excluir este lote?')) {
        ...
        try {
            localStorage.setItem('incubadora-batches', JSON.stringify(batches));
            renderBatches();
            showNotification('✅ Sucesso', 'Lote excluído!');
        } catch (e) {}
    }
}
...
function clearData() {
    if (confirm('Tem certeza que deseja limpar todos os dados? Isso exclui os lotes, mas mantém a chave da IA e o chat.')) {
        ...
        batches = [];
        renderBatches();
        showNotification('✅ Sucesso', 'Dados limpos!');
    }
}
